...
export const buildNarrativePrompt = (input: ContentInput): string => {
  const resolvedTravelMode = input.travelMode || (input.isFferryCrossing ? 'air' : 'land');
  const locationContext =
    resolvedTravelMode === 'air'
      ? `arriving by air to ${input.placeName}, ${input.region}, ${input.country}`
