...
- Calculates the next route point 20-30km east using land pathfinding (OSRM)
- Falls back to a single air jump to the nearest eastward landfall when land routing is blocked by sea
...
- `route` - Route planning (OSRM land paths + air fallback), city finding, geocoding (Overpass, Nominatim)
...
Route planner settings (optional) in `.env`:
- `OSRM_BASE_URL` (default in Docker: `http://osrm:5000`)
- `OVERPASS_BASE_URL` (default: `https://overpass-api.de/api/interpreter`)
- `OVERPASS_TIMEOUT_MS` / `OSRM_TIMEOUT_MS`
- `GEO_RETRY_*`, `GEO_CIRCUIT_*`, `GEO_CACHE_*` for retry/circuit/cache tuning
- `ROUTE_EAST_BEARING_MIN` (default: `65`)
- `ROUTE_EAST_BEARING_MAX` (default: `115`)
- `ROUTE_LANDFALL_SAMPLE_KM` (default: `25`)
- `ROUTE_LANDFALL_MAX_KM` (default: `1500`)
...
### 2.1 Bootstrap OSRM dataset (recommended for stable routing)

```bash
# Prepare local OSRM data and start the osrm service (development compose)
./scripts/osrm-bootstrap.sh dev

# Start osrm profile (if not already up)
...
# Start scheduler with local OSRM routing profile
docker-compose --profile scheduler --profile routing up -d scheduler osrm

# Run generator once (on demand)
docker-compose --profile routing run --rm scheduler sh -c "pnpm --filter @silicon-traveler/scheduler build && pnpm --filter @silicon-traveler/scheduler run-once -- --job generator"

# Run publisher once (on demand)
docker-compose --profile routing run --rm scheduler sh -c "pnpm --filter @silicon-traveler/scheduler build && pnpm --filter @silicon-traveler/scheduler run-once -- --job publisher"
```
